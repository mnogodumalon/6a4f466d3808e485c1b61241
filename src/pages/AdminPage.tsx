...
import { KontrollpunkteViewDialog } from '@/components/dialogs/KontrollpunkteViewDialog';
import { HygieneKontrolleDialog } from '@/components/dialogs/HygieneKontrolleDialog';
import { HygieneKontrolleViewDialog } from '@/components/dialogs/HygieneKontrolleViewDialog';
...
const KONTROLLPUNKTE_FIELDS = [
  { key: 'name', label: 'Bezeichnung', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'medizingeraet', label: 'Medizingerät' }, { key: 'hygienemittel', label: 'Hygienemittel' }, { key: 'notfallausstattung', label: 'Notfallausstattung' }, { key: 'sonstiges', label: 'Sonstiges' }, { key: 'temperatur', label: 'Temperaturkontrolle' }, { key: 'sterilisation', label: 'Sterilisation' }] },
  { key: 'sollwert', label: 'Sollwert / Grenzwert', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
];
const HYGIENEKONTROLLE_FIELDS = [
  { key: 'datum', label: 'Datum der Kontrolle', type: 'date/date' },
  { key: 'kontrollpunkt', label: 'Kontrollpunkt', type: 'applookup/select', targetEntity: 'kontrollpunkte', targetAppId: 'KONTROLLPUNKTE', displayField: 'name' },
  { key: 'messwert', label: 'Messwert / Ergebnis', type: 'string/text' },
  { key: 'in_ordnung', label: 'Ergebnis in Ordnung', type: 'bool' },
  { key: 'kontrolleur_vorname', label: 'Vorname (Kontrolleur/in)', type: 'string/text' },
  { key: 'kontrolleur_nachname', label: 'Nachname (Kontrolleur/in)', type: 'string/text' },
  { key: 'bemerkung', label: 'Bemerkung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'kontrollpunkte', label: 'Kontrollpunkte', pascal: 'Kontrollpunkte' },
  { key: 'hygiene_kontrolle', label: 'Hygiene-Kontrolle', pascal: 'HygieneKontrolle' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('kontrollpunkte');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'kontrollpunkte': new Set(),
    'hygiene_kontrolle': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontrollpunkte': return (data as any).kontrollpunkte as Kontrollpunkte[] ?? [];
      case 'hygiene_kontrolle': return (data as any).hygieneKontrolle as HygieneKontrolle[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'hygiene_kontrolle':
        lists.kontrollpunkteList = (data as any).kontrollpunkte ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'hygiene_kontrolle' && fieldKey === 'kontrollpunkt') {
      const match = (lists.kontrollpunkteList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontrollpunkte': return KONTROLLPUNKTE_FIELDS;
      case 'hygiene_kontrolle': return HYGIENEKONTROLLE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'kontrollpunkte': return {
        create: (fields: any) => LivingAppsService.createKontrollpunkteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateKontrollpunkteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteKontrollpunkteEntry(id),
      };
      case 'hygiene_kontrolle': return {
        create: (fields: any) => LivingAppsService.createHygieneKontrolleEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateHygieneKontrolleEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteHygieneKontrolleEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'kontrollpunkte' || dialogState?.entity === 'kontrollpunkte') && (
        <KontrollpunkteDialog
          open={createEntity === 'kontrollpunkte' || dialogState?.entity === 'kontrollpunkte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'kontrollpunkte' ? handleUpdate : (fields: any) => handleCreate('kontrollpunkte', fields)}
          defaultValues={dialogState?.entity === 'kontrollpunkte' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Kontrollpunkte']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Kontrollpunkte']}
        />
      )}
      {(createEntity === 'hygiene_kontrolle' || dialogState?.entity === 'hygiene_kontrolle') && (
        <HygieneKontrolleDialog
          open={createEntity === 'hygiene_kontrolle' || dialogState?.entity === 'hygiene_kontrolle'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'hygiene_kontrolle' ? handleUpdate : (fields: any) => handleCreate('hygiene_kontrolle', fields)}
          defaultValues={dialogState?.entity === 'hygiene_kontrolle' ? dialogState.record?.fields : undefined}
          kontrollpunkteList={(data as any).kontrollpunkte ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['HygieneKontrolle']}
          enablePhotoLocation={AI_PHOTO_LOCATION['HygieneKontrolle']}
        />
      )}
      {viewState?.entity === 'kontrollpunkte' && (
        <KontrollpunkteViewDialog
          open={viewState?.entity === 'kontrollpunkte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'kontrollpunkte', record: r }); }}
        />
      )}
      {viewState?.entity === 'hygiene_kontrolle' && (
        <HygieneKontrolleViewDialog
          open={viewState?.entity === 'hygiene_kontrolle'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'hygiene_kontrolle', record: r }); }}
          kontrollpunkteList={(data as any).kontrollpunkte ?? []}
        />
      )}
