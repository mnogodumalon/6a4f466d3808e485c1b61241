...
import type { Kontrollpunkte, HygieneKontrolle, CreateKontrollpunkte, CreateHygieneKontrolle } from '@/types/app';
...
export class LivingAppsService {
  // --- KONTROLLPUNKTE ---
  static async getKontrollpunkte(): Promise<Kontrollpunkte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KONTROLLPUNKTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kontrollpunkte[];
    return enrichLookupFields(records, 'kontrollpunkte');
  }
  static async getKontrollpunkteEntry(id: string): Promise<Kontrollpunkte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KONTROLLPUNKTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kontrollpunkte;
    return enrichLookupFields([record], 'kontrollpunkte')[0];
  }
  static async createKontrollpunkteEntry(fields: CreateKontrollpunkte) {
    return callApi('POST', `/apps/${APP_IDS.KONTROLLPUNKTE}/records`, { fields: cleanFieldsForApi(fields as any, 'kontrollpunkte') });
  }
  static async updateKontrollpunkteEntry(id: string, fields: Partial<CreateKontrollpunkte>) {
    return callApi('PATCH', `/apps/${APP_IDS.KONTROLLPUNKTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kontrollpunkte') });
  }
  static async deleteKontrollpunkteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KONTROLLPUNKTE}/records/${id}`);
  }

  // --- HYGIENE_KONTROLLE ---
  static async getHygieneKontrolle(): Promise<HygieneKontrolle[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HYGIENE_KONTROLLE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as HygieneKontrolle[];
    return enrichLookupFields(records, 'hygiene_kontrolle');
  }
  static async getHygieneKontrolleEntry(id: string): Promise<HygieneKontrolle | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HYGIENE_KONTROLLE}/records/${id}`);
    const record = { record_id: data.id, ...data } as HygieneKontrolle;
    return enrichLookupFields([record], 'hygiene_kontrolle')[0];
  }
  static async createHygieneKontrolleEntry(fields: CreateHygieneKontrolle) {
    return callApi('POST', `/apps/${APP_IDS.HYGIENE_KONTROLLE}/records`, { fields: cleanFieldsForApi(fields as any, 'hygiene_kontrolle') });
  }
  static async updateHygieneKontrolleEntry(id: string, fields: Partial<CreateHygieneKontrolle>) {
    return callApi('PATCH', `/apps/${APP_IDS.HYGIENE_KONTROLLE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'hygiene_kontrolle') });
  }
  static async deleteHygieneKontrolleEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HYGIENE_KONTROLLE}/records/${id}`);
  }
